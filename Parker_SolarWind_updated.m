...
r_desired = 1:10:250; % [solar redii]
    ...
    filename = sprintf('models_density.txt');
    fileID = fopen(filename, 'w');
    ...
    filename = sprintf('models_frequency.txt');
    fileID = fopen(filename, 'w');
    fprintf(fileID, 'Distance (R_solar)\t Newkirk (Hz) \t Saito (Hz) \t Leblanc (Hz) \t Vrsnak (Hz) \t\n');
...
theo_filename = 'models_density.txt';
...
% Load theoretical model data
theo_filename = 'models_frequency.txt';
theo_data = dlmread(theo_filename, '', 1, 0);
